...
class HPatrolNovaStack(Stack):
    ...
    def _createLambdasStructure(self) -> None:
        ...
        trafficlandSrcDir = f"{self.cwd}/stacks/generators/trafficlandParser"

        logger.info("Constructing lambda build dirs")
        outputDir = f"{self.cwd}/.lambdaBuild"

        trafficlandDir = outputDir + "/trafficland"

        pathlib.Path(trafficlandDir).mkdir(parents=True, exist_ok=True)
        copy_tree(trafficlandSrcDir, trafficlandDir)
        copy_tree(commonSrcDir, trafficlandDir)

        logger.info("Lambda build dirs created")

        self.buildDirs = {
            "trafficlandBuildDir": trafficlandDir
        }
