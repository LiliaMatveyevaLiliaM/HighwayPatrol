...
class HPatrolSingaporeStack(Stack):
    ...
    def __init__(self,
        ...
        **kwargs) -> None:
        ...
        self._createHangEmHighLambda()
        self._createTheGauntletLambda()


    def _createHangEmHighLambda(self) -> str:
        theLambda = PythonFunction(self, "hangemhighLambda",
            description="Collects and parses Hang Em High site to create aimpoints",
            entry=self.buildDirs["hangEmHighBuildDir"],
            index="src/python/main.py",
            role=self._lambdaRole,
            runtime=Runtime.PYTHON_3_13,
            handler="lambdaHandler",
            function_name=self.baseStackName + "_HangEmHigh",
            memory_size=1024,
        ...
        logger.info("Hang Em High lambda defined")
    ...
    def _createTheGauntletLambda(self) -> str:
        theLambda = PythonFunction(self, "thegauntletLambda",
            description="Collects and parses The Gauntlet site to create aimpoints",
            entry=self.buildDirs["theGauntletBuildDir"],
            index="src/python/main.py",
            role=self._lambdaRole,
            runtime=Runtime.PYTHON_3_13,
            handler="lambdaHandler",
            function_name=self.baseStackName + "_TheGauntlet",
            memory_size=1024,
            # Using "unreserved account concurrency"
            timeout=Duration.minutes(15),
            retry_attempts=0,
        ...
        logger.info("The Gauntlet lambda defined")
    ...
    def _createLambdasStructure(self) -> None:
        ...
        hangEmHighSrcDir = f"{self.cwd}/stacks/generators/hangEmHighParser"
        theGauntletSrcDir = f"{self.cwd}/stacks/generators/theGauntletParser"

        logger.info("Constructing lambda build dirs")
        outputDir = f"{self.cwd}/.lambdaBuild"

        hangEmHighDir = outputDir + "/hangEmHighParser"
        theGauntletDir = outputDir + "/theGauntletParser"

        pathlib.Path(hangEmHighDir).mkdir(parents=True, exist_ok=True)
        copy_tree(hangEmHighSrcDir, hangEmHighDir)
        copy_tree(commonSrcDir, hangEmHighDir)

        pathlib.Path(theGauntletDir).mkdir(parents=True, exist_ok=True)
        copy_tree(theGauntletSrcDir, theGauntletDir)
        copy_tree(commonSrcDir, theGauntletDir)

        logger.info("Lambda build dirs created")

        self.buildDirs = {
              "hangEmHighBuildDir": hangEmHighDir
            , "theGauntletBuildDir": theGauntletDir
        }
