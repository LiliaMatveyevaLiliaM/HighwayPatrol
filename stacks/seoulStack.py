...
class HPatrolSeoulStack(Stack):
    ...
    def __init__(self,
        ...
        **kwargs) -> None:
        ...
        self._createHKCamsLambda()


    def _createHKCamsLambda(self) -> str:
        theLambda = PythonFunction(self, "hkcamsLambda",
            description="Collects and parses HK Cams site to create aimpoints",
            entry=self.buildDirs["hkCamsBuildDir"],
    ...
    def _createLambdasStructure(self) -> None:
        ...
        HKCamsSrcDir = f"{self.cwd}/stacks/generators/xenonParser"

        logger.info("Constructing lambda build dirs")
        outputDir = f"{self.cwd}/.lambdaBuild"

        hkCamsDir = outputDir + "/xenonParser"

        pathlib.Path(hkCamsDir).mkdir(parents=True, exist_ok=True)
        copy_tree(HKCamsSrcDir, hkCamsDir)
        copy_tree(commonSrcDir, hkCamsDir)

        logger.info("Lambda build dirs created")

        self.buildDirs = {
              "hkCamsBuildDir": hkCamsDir
        }
